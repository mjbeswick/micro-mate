...
**`src/micromate/pgn.py`** — bridges the engine's `(row, col)` `Move` with `python-chess`. PGN is **8×8 only** (`is_pgn_compatible`). Replay stops at the first move our engine can't model (castling, en passant), which is fine for prefix replay but means `import_pgn` may return a truncated game.
